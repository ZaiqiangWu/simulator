...
Obj::Obj(const string file):obj_file(file)
{
	...
	if(!texture_file.empty()) // load .png
	{
		string path = obj_file.substr(0,obj_file.find_last_of("/"));
	...
	// after loading data, unify it first for simulation and render
	unified();
